...
export function PipelineHeatmap() {
  const maxInflight = Math.max(...PIPELINE_STAGES.map((s) => s.inflight));
    ...
    <div className="space-y-4">
            ...
            return (
              <div key={s.id} className="flex-1 flex flex-col items-center gap-1">
                <div className="flex-1 w-full flex items-end relative">
                  <div
                    className={`w-full rounded-t-md bg-gradient-to-t ${barColor} relative shadow-md transition-all`}
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-emerald-100/60 backdrop-blur-sm rounded-lg p-3 border border-emerald-300/60">
          ...
          <div className="text-xl font-bold text-gray-900">{PIPELINE_STAGES.reduce((a, s) => a + s.inflight, 0)}</div>
        </div>
        <div className="bg-rose-100/60 backdrop-blur-sm rounded-lg p-3 border border-rose-300/60">
          ...
          <div className="text-xl font-bold text-gray-900">
            {PIPELINE_STAGES.filter((s) => s.bottleneck).map((s) => s.label).join(', ') || 'None'}
          </div>
        </div>
      ...
      <div className="text-[10px] text-gray-600 italic">
        TODO: subscribe to os:stream:* XLEN counts via gateway SSE for live updates.
      </div>
    </div>
  );
}
